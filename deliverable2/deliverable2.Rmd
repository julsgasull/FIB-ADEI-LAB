...
```{r}
names(df)
vars_res<-names(df)[c(15,27)] # Total_amount, TipIsGiven
vars_quantitatives<-names(df)[c(3:10,12,20:22)] # Pickup_longitude, Pickup_latitude, Dropoff_longitude, Dropoff_latitude, Passenger_count, Trip_distance, Fare_amount, Extra, Tip_amount, tlenkm, traveltime, espeed
vars_categorical<-names(df)[c(1,2,16:17,19,25,28)] # VendorID, RateCodeID, Payment_type, Trip_type, period, Trip_distance_range, passenger_groups
...
## ho hem de parlar, jo no ho posaria.
...
text(res.pca$quali.sup$coord[,1],res.pca$quali.sup$coord[,2],labels=names(res.pca$quali.sup$coord[,1]),col="cadetblue1",cex=0.8)
...
df$hcpck<-factor(df$hcpck,labels=paste("kHP-",1:5))
df$claKM<-factor(df$claKM,levels=c(1,5,3,2,4),labels=c("kKM-1","kKM-5","kKM-3","kKM-2","kKM-4"))
