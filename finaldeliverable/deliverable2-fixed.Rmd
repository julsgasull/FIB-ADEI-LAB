...
* They all have MTA_tax to yes
* They don't have any tip, neither tolls
* They all have improvement_surcharge to yes
* The total amount varies bwtween 3.3 and 4.8
* The majority of them are paid with cash
* The majority of them are during afternoon
* All of them have a distance of 1km
* The duration of each trip does not exceed two minutes
* Almost every tirp is done only with one passenger
...
* They all have MTA_tax to yes
* They all have improvement_surcharge to yes
* The total amount varies bwtween 64.34 and 128.76
* The majority of them are paid with credit card
* The majority of them are during night
* The trip distance varies bwtween 29.31 and 48.28 km
* The duration of each trip is between half and hour and an hour
* Almost every tirp is done only with one passenger
...
* They all have MTA_tax to yes
* They have tolls amount equal to 5.54
* They all have improvement_surcharge to yes
* The total amount varies bwtween 21.84 and 64.34 
* The majority of them are paid with credit card
* The majority of them have a long distance
* The duration of each trip is bwtewwn 10 and 30 minutes
* Almost every tirp is done only with one passenger
...
* They all have MTA_tax to yes
* They all have improvement_surcharge to yes
* The total amount varies bwtween 10 and 41
* The majority of them are paid with credit card
* The majority of them are during afternoon
* The trip distance varies bwtween 1 and 10 km
* The duration of each trip is between 12min and an hour
* Almost every tirp is done only in groups
...
Finally, if we take a look at the **categories** we see that for the Trip_distance_range category long distance trips show a mean 2.25 units over the global mean and short distance ones show a mean -1.95 units under the global mean, so we can reject the H0 done in the t.Student test.
...
For the **quanlitatives**, we see that period is the category that is more related with 0.036, even though it is not a big relation.
...
It may not look very good, but our TipIsGiven variable is within the center of mass of the observation cloud.
...
Finally RateCode ID 1 does not provide anything in particular.
...
According to the previous plot, the optimal number of clusters per k-means is 1, so we guess maybe something is wrong or missing. As the teacher told us, we should interpret 4.
...
The first thing we can see from the summary is that we have a chi square statistic of 6168.477, great enough to reject the H0, which means the intensity of the relation is high. If we take a look at the variances from the different dimensions, we can see that all together sum more than 1. 
...
Following the kaiser kriteria and the value got in the output, we should retain dimensions with a variance greater than 0.3335754. In this case, the first dimension fulfills this because its variance is 0.744, but it is not enough to work with data so, we would choose 2 o 3 dimensions for this case.
...
```{r}
names(df[,c(1:32)])
names(df[,c(3:10, 12:13, 15, 18, 20:22)])
names(df[,c(27,31)])
res.mca_all <- MCA(
  df[,c(1:32)], 
...
# # $`1`
# #                                         Cla/Mod   Mod/Cla     Global 
# # f.target.tip_is_given=tip_no          1.0108052 100.00000 62.4102676
# # f.target.tip_is_given=tip_yes         0.0000000   0.00000 37.5897324
# # 
# # $`2`
# #                                  Cla/Mod     Mod/Cla     Global 
# # nothing to see here
...
# # $`4`
# #                                  Cla/Mod     Mod/Cla     Global 
# # f.target.tip_is_given=tip_yes         48.263889   41.70 37.5897324 
# # f.target.tip_is_given=tip_no          40.641338   58.30 62.4102676 
# # 
# # $`5`
# #                                     Cla/Mod    Mod/Cla    Global 
# # f.target.tip_is_given=tip_no           3.06727083 83.0188679 62.410268 
# # f.target.tip_is_given=tip_yes          1.04166667 16.9811321 37.589732 
```
