...
We consider, according to the generalized Kaiser theorem, all those dimensions such that their eigenvalue is greater than the mean. We see that the average gives us 0.1428571. Therefore, we will take up to dimension 6, which represents the 62.02% of the sample.
```{r}
mean(res.mca$eig[,1])
...
As we can see in the plot "Variables representation", the correlation between the f.payment_type factor taking into account the eta2 and the second factorial axis is a value greater than 0.5. On the other hand, we can see that something similar happens with the f.trip_type factor and f.rate_code_id in dimension 1.
```{r}
...
As we can see, the “no paid” category (f.payment_type variable) is the one farthest from the center of the plot (in dimension 2). The farther from the center of gravity, the more rarely this feature value appears in the sample represented by the dimension. In addition, we see that in dimension 1 we also have two extremes, the "code_rate_id_other" category ("f.code_rate_id" variable) and the "trip_dispatch" category ("f.trip_type" variable). As we have said, this means that these categories are rarely represented in this dimension.
...
* f.trip_type=trip_dispatch (1.74)
* f.rate_code_id=rate_code_id_other (1.66)
...
* f.payment_type=no paid (1.86)
* f.vendor_id=vendor_id_mobile (0.26)
...
* q.trip_distance          0.53263364
* q.fare_amount            0.53136451
* q.tlenkm                 0.51800587

We also see that they do not contribute much given the numbers.

However, there is a little more inverse relationship with 

* q.extra                 -0.62829962

Regarding the **qualitative** variables, the new relationship is as follows:

* f.period                0.475926312
* qual.pickup             0.570138464
* qual.dropoff            0.554006489 
* f.trip_distance_range   0.297157633

If we look at the **categories**, we see that the most related are,

* f.hour=f.hour_other                      0.5642223317  
* f.extra=extra_no                         0.3329791331
* f.trip_distance_range=trip_dist_long     0.4386776525
* f.dist=(5.5, 30]                         0.5076041966 
...
* q.trip_distance          0.64383966 
* q.tlenkm                 0.64273304 
* q.target.total_amount    0.63306027 
* q.fare_amount            0.59446156

For the second dimension we see that for the **qualitative** variables the most positively related, from more to less, are:

* f.period                0.3956288348 
* qual.pickup             0.4497408628 
* qual.dropoff            0.4541554265
* f.trip_distance_range   0.4677644156 
* f.extra                 0.3202538147 
* f.dist                  0.4694364650
* f.cost                  0.4265269270 
...
* f.dist=(5.5, 30]                         0.59673706 
* f.extra=extra_yes                        0.27704085
* f.trip_distance_range=trip_dist_long     0.51551902
...
We can see in the output that the variable that appears is slightly over represented in the clusters. We can notice that **q.target.total_amount** is over represented with 0.04 units over the global mean. So it is practically the same as the global mean.
...
The quality of this reduction if of 57.59%.
...
We have a concordance of the 39.77% so we can say that they are different, if we had a greater concordance, this would mean that they would be more similar.
...
We have a concordance of the 49.16% so we can say that they are different, if we had a greater concordance, this would mean that they would be more similar.
...
## ## $`3`
## ##                                         Cla/Mod    Mod/Cla    Global
## ## f.target.tip_is_given=tip_no         0.7319624    84        62.41027
## ## f.target.tip_is_given=tip_yes        0.2314815    16        37.58973 
## ##
## ## $`4`
## ##                                          Cla/Mod   Mod/Cla    Global
