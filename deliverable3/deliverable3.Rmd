...
vars_dis<-names(df)[c(1:2,16,19,27:32)]; # si les fem --> posar: f.speed, f.nbpass, f.cost, f.len1, f.dist, f.tt, f.hour
...
Normal data should have 0 skewness: we see that our data is right skewed (3.16).
...
Normal data should be 3. We have 21, so, in this case, our data is not normal.
...
## Third model: if few explanatory variables are available: take all of them
```{r}
vars_cexp
...
Model_1 explains 92.93% of the variability of the target. We also see, according to the F-statistic, that it should be rejected.
...
Note that trip_distance still has a vif greater than 5 (11.063910).

```{r}
summary(model_1_bic)
```

However, we see that it continues to explain much of the variability of our target (92.02%).
...
We see that the explainability is now 92.9%.
...
We see that the explainability is now 92.57%.
...
We see that the explainability is now 85.23%.
...
We see that the explainability is now 85.17%
...
Veient els resultats, podem dir que:
* No hi ha normalitat
* I, pel que fa el gràfic de Residuals vs Leverage, les nostres variables estan dins del model de R, però no és garie fiable, així que no ens serveix molt.

Tot això és donat a que la nostra variable target ja no estava distribuida de forma normal. Per solucuionar-ho, li apliquem el logaritme:

NOTA: a l'aplicar l'algoritme ens ha donat el següent error: 
...
m24<-lm(log(target.total_amount) ~ q.extra + q.tip_amount + q.tolls_amount + poly(q.tlenkm,2),data=df)
summary(m24)
Anova(m24)
