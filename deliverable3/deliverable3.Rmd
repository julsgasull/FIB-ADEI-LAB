...
* Veiem que les variables més influents, en el nostre cas, és q.hour.
* Veiem que la "residual deviance! ésde 1386.6  on 1472  degrees of freedom.

```{r}
vif(model_20)
```

Veiem que tenim algunes variavles amb vifs molt alts:

* q.trip_distance (66.67)
* q.tlenkm (63.09) --> correlated with the preious
* q.fare_amount (9.48)
* q.traveltime (5.16)

### Model 21

*NOTA: som conscients que no hi hauria d'haver factors en aquest apartat, però hem decidit posar-los donat que les seves variables originals (e.g. mta_tax) les vam sobreescriure per fer els factors.*

Sabem que no hi ha colinearitat. Fem un nou model:
...
Podem transformar tlenkm i treure improvement_subcharge per tal de tenir els vifs més baixos:
...
Donat que ens qeda un model bastant pobre, agafarem també la variable q.extra, per tal de poder extreure més informació, com per exemple, poder fer els marginal plots:
...
* Per a f.mta_tax: veiem que, si f.mta_tax="Yes", és més probable que target.tip_is_given="Yes".
* Per a q.extra: com hem dit, aquesta variable no afecta molt al target, però la incloem per poder fer més plots. Com a màxim es podria dir que és "inversament" proporcional al target.

```{r}
marginalModelPlots(model_23)
```

Veiem que q.extra és candidata a ser factor.

```{r}
residualPlots(model_23)
```

Veiem que els smoothers són relativament, plans, així que podem dir que, de moment, tot va bé.


Anem, però, a proposar un model que ens doni més joc:
...
* q.tlenkm: 
  + veiem que el smoother és pla, per tant, està bé
  + les formes "rares" que apareixen són pel model de resposta binària
* q.extra:
  + veiem que el smoother és pla, per tant, està bé
* q.espeed: 
  + veiem que el smoother és pla, per tant, està bé
  + les formes "rares" que apareixen són pel model de resposta binària
* tot el model
  + veiem que el smoother no està del tot recte, però, tal i com hem dit a classe, podem treballar amb desajust al model, donat que és un tema massa dens.
...
Ens hem d'assegurar que no tenim cap variable amb un efecte net que no sigui significatiu. 

Així doncs, anem a refer el model:
...
Amb Anova(model_24), veiem que es compleix.


## Factors //See if you need to replace a number with its equivalent factor

Mirem si alguna de les variables numèriques la podem substituir per un factor.

El que primer fariem seria canviar "q.mta_tax" (en cas que existís) per "f.mta_tax. Com que ja està posat amb un factor, ens saltem aquest pas.

Donat que l'altra variable per posar com a factor depén d'un polinomi, la deixem com està. Tot i així, el codi que s'hauria de fer en cas d'un nou model amb un factor afegit seria el següent:
...
Gràcies a BIC(model_24, model_25) podriem veure els canvis que ens generaria el nou model. Com menor sigui el BIC, millor model tindrem. Recordem també que, en cas d'haver fet un intercanvi de variable quantitativa a factor, no ho podriem fer amb anova, donat que hi ha un intercanvi i, per tant, cap model és major que l'altre.

## Decision //Add to the best model of step 2, the main effects of the factors and retain the significant net effects.

Decidim que ens quedem amb el model_25.
...
Ara que ja tenim un model definit, anem a fer interaccions amb totes les variables factors que creiem importants:
...
Treiem les variables que no són significatives:
```{r}
model_27 <- step(model_26, k=log(nrow(dffwork)))
```

Pel que veiem, ens deixa només amb la taxa, però, per tenir-hi més joc, deixarem el que teniem abans.
...
Ens quedem amb el 25.

Podem veure ara els efectes d'aquest:
```{r}
plot(allEffects(model_25))
```

* Veiem que només es dona propina en un cert rang de km fets, donat que, per pocs km, no té sentit donar-ne i, per molts, és massa.
* Com hem comentat abans, és més fàcil donar propina sú hi ha hagut taxa.

### factor-factor

Donat que a la pràctica ens demanen fer interaccions entre factors, ho farem encara que els resuptats no siguin realistes:
...
Conclusió: seguim en que el millor model és el model_25.

Ara, doncs, anem a fer-ne diagnosi:
...
Veiem que són mostres que tenen mta_tax, però que en canvi, no tenen tip.


Anem ara, a determinar quines són les observacions potencialment influents:
...
* be long (distance) but short (time)
                                                                                                      ...
                                                                                                      Ara, per dcidir les influències, anem a mirar les distàncies de cook:             
