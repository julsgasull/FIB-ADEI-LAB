...
PER PREGUNTAAAAAAR-----------------------------------------------------------------
```{r}
df$target.log_total_amout <- log(df$target.total_amount)
sel<-which(df$target.log_total_amout=="-Inf")
df[sel,"target.log_total_amout"]<-0 

model_6 <- lm(
  target.log_total_amout~ 
    q.passenger_count +
    q.extra +
    q.tip_amount +
    q.tolls_amount +
...
We see that when doing the logarithm, the coefficient of determination is getting lower and lower, now it is 76.15%. We have seen that it has gotten worse than the previous model. Therefore, we discard it. We will work with model_5.
