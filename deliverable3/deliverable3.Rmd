...
fit.tip_is_given0 <- factor(ifelse(fit0<0.5,0,1), labels=c("fit.no", "fit.yes"))
tt0 <- table(fit.tip_is_given0, dffwork$target.tip_is_given); tt0
100*sum(tt0[1,2]/sum(tt0))
100*(tt0[2,2]/(tt0[2,2] + tt0[1,2])) # recall (sensitivity)
100*(tt0[1,1]/(tt0[1,1] + tt0[2,1])) # specificity
100*(tt0[2,2]/(tt0[2,1]+ tt0[2,2])) # precision
```

We have an accuracy of 82.91%.
