...
# we set the 0 as an error because it is not possible to 
# have a trip without passengers
sel<-which(df$Passenger_count == 0)
ierrs[sel]<-ierrs[sel]+1
# names(df)
jerrs[10]<-length(sel)
# sel
# sel contains the rownames of the individuals with "0" 
# as value for passengers
...
# we see on the summary that there are not NA values, so we 
# proceed to the outlier and error detection.

# Outlier detection
## in order to evalute or data, we decide to set the maximum 
## trip distance to 30, so we proceed to delete the outliers.
...
# Error detection
## we decide that an incorrect trip distance is the one with 0 
## miles or less. In order to be aware of this error we store 
## it at ierrs, and jerrs. ierrs stores the number of errors in
## a row, and jerrs stores the total amount of errors in a variable.
...
# Errors and outliers
## now, we set NA values in order to remove errors and outliers 
## from the dataset
setNA<-which((df$Trip_distance<=0) | (df$Trip_distance > 30))
df[setNA,"Trip_distance"]<-NA

# Caterogial variable for Trip_distance
## we are going to set a categorical variable for the Trip_distance 
## range. 
## we decided to create 3 levels: "Short_dist", "Medium_dist" and 
## "Long_dist".
### - Short_dist <= 2.5
...
# it is a quantitive variable.  
# non-possible values will be recoded as errors, 
# so will be transformed to NA.
...
# non-possible values are replaced by NA, missing value 
# symbol in R

# Which trips are not running in New-York?
## consider if, at least, one of the pick-up and drop-off 
## points belong to New-York area. if not, this trip is 
## an "out-of-scope" individual and has to be eliminated 
## of the basis. Nevertheless, you have to justify this 
## elimination and count how many individuals were in 
## this situation.
## look at that!! possibly, starting from the outliers... 
## "0" is missing value, outliers can help to detect trips 
## running outside of New York...

# we are deleting trips from outside New York. This means 
# we are not using longitudes bigger than -73.80 and smaller 
# than -74.02
...
# we are deleting trips from outside New York. This means 
# we are not using latitudes bigger than 40.54 and smaller 
# than 40.86
...
# we are deleting trips from outside New York. This means 
# we are not using longitudes bigger than -73.80 and 
# smaller than -74.02
...
# we are deleting trips from outside New York. This means 
# we are not using latitude bigger than 40.54 and smaller 
# than 40.86
...
# non-possible values are replaced by NA, 
# missing value symbol in R
...
# we decide to set outliers for fare amounts 
# bigger than 60, because the majority of the 
# values are concentrated between 0 and 60.
...
# we execute table in order to see every different 
# value in the sample
table(df$Extra)

# as it is a price related variable, negative values 
# should be treated as errors, and the other values 
# are the ones defined for this variable, so there 
# are not outliers.
# df[which(df[, "Extra"] < 0),]
...
# we know that this surcharge was leived in 2015, so we 
# need to check if the 0 values correspond to trips before 
# this year. That is what we are going to do.
...
# we see that the 0 individuals are errors, so we 
# proceed to set them has NA.
...
# we proceed to check if the 0 values are related 
# with payment_type = "Credit card" and the passenger 
# did not tip.
df$CashTips[(df$Tip_amount > 0) & (df$Payment_type == "Cash")] = 1
df$CashTips[(df$Payment_type == "Credit card")] = 0
table(df$CashTips)

# we see that we have correct data, so we proceed 
...
# we see that there are not negative values, so we 
# do not have errors. We proceed now to the outlier 
# detection.
...
# as we see in the boxplot and the table, the majority 
# of the individuals are 0, so the values bigger than 
# 0 will be outliers.
...
# we proceed to check if total amount is correct 
# summing the other variables and checking negatives 
# values:
...
### Number of errors, outliers and rank of each variable

### 1. VendorID
Being categorical, there are no mistakes or outliers. Thus, the rank is equal to 0.

### 2. lpep_pickup_datetime / pickup
Being categorical, there are no mistakes or outliers. Thus, the rank is equal to 0.

### 3. lpep_dropoff_datetime / dropoff
Being categorical, there are no mistakes or outliers. Thus, the rank is equal to 0.

### 4. Passenger_count
```{r}
jerrs[10] # number of errors (including inconsistencies)
jouts[10] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[10] + mis1$mis_col$mis_x[10]) / 5000; rank
```

### 5. Trip_distance
```{r}
jerrs[11] # number of errors (including inconsistencies)
jouts[11] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[11] + mis1$mis_col$mis_x[11]) / 5000; rank
```

### 6. Pickup_longitude
```{r}
jerrs[6] # number of errors (including inconsistencies)
jouts[6] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[6] + mis1$mis_col$mis_x[6]) / 5000; rank
```

### 7. Pickup_latitude
```{r}
jerrs[7] # number of errors (including inconsistencies)
jouts[7] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[7] + mis1$mis_col$mis_x[7]) / 5000; rank
```
...
### 10. Dropoff_longitude
```{r}
jerrs[8] # number of errors (including inconsistencies)
jouts[8] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[8] + mis1$mis_col$mis_x[8]) / 5000; rank
```

### 11. Dropoff_latitude
```{r}
jerrs[9] # number of errors (including inconsistencies)
jouts[9] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[9] + mis1$mis_col$mis_x[9]) / 5000; rank
```
...
### 13. Fare_amount
```{r}
jerrs[12] # number of errors (including inconsistencies)
jouts[12] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[12] + mis1$mis_col$mis_x[12]) / 5000; rank
```

### 14. Extra
```{r}
jerrs[13] # number of errors (including inconsistencies)
jouts[13] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[13] + mis1$mis_col$mis_x[13]) / 5000; rank
```

### 15. MTA_tax
CATEGORICAAAL????
```{r}
jerrs[14] # number of errors (including inconsistencies)
jouts[14] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[14] + mis1$mis_col$mis_x[14]) / 5000; rank
```

### 16. Improvement_surcharge
```{r}
jerrs[18] # number of errors (including inconsistencies)
jouts[18] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[18] + mis1$mis_col$mis_x[18]) / 5000; rank
```

### 17. Ehail_fee
All values of this variable are NA's, so we do not take it into account.

### 18. Tip_amount
```{r}
jerrs[15] # number of errors (including inconsistencies)
jouts[15] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[15] + mis1$mis_col$mis_x[15]) / 5000; rank
```

### 19. Tolls_amount
```{r}
jerrs[16] # number of errors (including inconsistencies)
jouts[16] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[16] + mis1$mis_col$mis_x[16]) / 5000; rank
```

### 20. Total_amount
```{r}
jerrs[19] # number of errors (including inconsistencies)
jouts[19] # number of outliers

# rank variables according the sum of missing values (and errors).
rank <- (jerrs[19] + mis1$mis_col$mis_x[19]) / 5000; rank
```
...
```{r}
table(imis)
barplot(table(imis),main="Missings per individual Barplot",col = "DarkOrchid")
```
...
```{r}
# table(ierrs)
barplot(table(ierrs),main="Errors per individual Barplot",col = "DarkOrchid")
```

### Number of outliers
```{r}
# table(iouts)
barplot(table(iouts),main="Outliers per individual Barplot",col = "DarkOrchid")
```
...
# sum
for (m in imis) {total$missings <- total$missings + m}; total$missings
for (o in iouts) {total$outliers <- total$outliers + o}; total$outliers
for (e in ierrs) {total$errors <- total$errors + e}; total$errors
total
```
